...
## B11 — CI workflow assertion for Supabase Edge Function deploy coverage

- **What:** Replace the manually-maintained per-function deploy step list in `.github/workflows/deploy-functions.yml` with either (a) a dynamic loop iterating over `supabase/functions/*` directories and invoking `supabase functions deploy <name>` for each, or (b) a CI assertion step that diffs the set of `supabase/functions/*` directories against the set of `supabase functions deploy` invocations in the workflow file and fails the build on mismatch. Option (a) is cleaner; option (b) is the lighter forward-compatible step if the workflow needs per-function configuration that doesn't fit a loop.
- **Why:** Slice 7 introduced `supabase/functions/invite-user/` but the workflow was not updated to add a "Deploy invite-user" step. Result: the function existed in the repo but was not deployed to cloud Supabase, leaving the admin invite flow non-functional in production until the gap was caught manually post-deploy. The audit framework reviews code, not deployment state, so it did not surface this. Mirrors the "CI is the only path to cloud" lock established in CLAUDE.md §6 — extending that lock to assert _which_ functions reach cloud, not just that CI is the route.
- **Trigger:** B7 pre-launch test sweep, OR the next Edge Function added after Slice 7 (Slice 8's demo signup may add a function; if so, fold B11 into that slice so the new function doesn't repeat the Slice 7 deploy gap). Whichever fires first.
