...
The prompt target and the enforced bound are deliberately different numbers **and different units**. The prompt asks for 220–280 words (`SUMMARY_TARGET_*_WORDS`); Zod allows 200–3000 characters (`SUMMARY_MIN/MAX_CHARS`). Anthropic does not honor `minLength`/`maxLength`, and a model cannot count its own characters. Production established this in two steps: with prompt and bound both at 2000, summaries landed at 2075, 2268, 2315, and 2352 and failed all four meetings; with the prompt lowered to 1200–1800 characters against a 2500 bound, the next summary came back at 2646 — _longer_ than before. A character target is not an instruction the model can follow, so output length tracks the amount of meeting rather than the stated figure.

Words are a unit the model approximates well. 220–280 words is ~1300–1700 characters, and the 3000-character bound clears the observed 2646 with margin. The worker's single correction retry (`apps/worker/src/pipeline/length-retry.ts`) covers the tail. A summary that still exceeds 3000 fails the row loudly for manual re-run; it is not silently truncated, because shortening published journalism without a human in the loop is worse than a visible failure.

The prompt also asks for two to three blank-line-separated paragraphs with a stated shape (headline outcome, then substantive items in order, then what happens next). The reader splits on blank lines and renders one `<p>` per paragraph.
...
**Public read, per publication (ADR 0024).** `publications.public_read boolean not null default false` opts a single publication into anonymous reading. Anon SELECT policies on `publications`, `towns`, `boards`, `meetings`, and `segments` mirror the membership-scoped authenticated policies with the membership subquery replaced by the flag; meetings keep the `status = 'published'` gate. `search_segments` is granted to `anon` and, being SECURITY INVOKER, scopes its results through the same policies.

The default is load-bearing: every existing and future publication is private unless someone deliberately flips the flag, so the paid, invitation-only path is what happens when nobody does anything. Turning public access off again is one `UPDATE` — no deploy, no migration. Grants to anon are column-scoped, excluding `meetings.audio_url`, `asr_transcript_id`, `transcript_url`, `last_error`, `failed_at`, and `segments.embedding`.

The signed-out root route falls through to a `public_read` publication before showing the no-access empty state, so an anonymous visitor lands on readable content rather than a dead end. Admin surfaces remain authenticated and membership-gated regardless of the flag.
...
## B9 — NI-009 resolution: Deno-side import wiring for `packages/shared` in search Edge Function

- **What:** Replace the inline OpenAI embeddings-response Zod schema (`supabase/functions/search/index.ts:43-55`) and the `EMBEDDING_MODEL` / `EMBEDDING_DIMENSIONS` constants (`supabase/functions/search/index.ts:21-22`) with imports from `packages/shared/src/embedding/`. Use the `npm:` specifier path already established for Zod at `supabase/functions/search/index.ts:15`. Verify Edge Function redeploy and search round-trip end-to-end. On success, mark NI-009 as Resolved (manual registry edit per `docs/workflows/build-cycle.md` routing), close NI-020 as Resolved at the same time (its revisit trigger is this slice shipping), and remove this Backlog entry. On infeasibility, document the Deno-side blocker and update NI-009 reasoning with a third-Edge-Function revisit trigger.
- **Why:** NI-009's second-Edge-Function revisit trigger fired in Slice 6. `supabase/functions/search/index.ts` is the second Edge Function and redefines both the OpenAI embeddings-response Zod schema and the model/dimensions constants inline rather than importing from `packages/shared/src/embedding/`. The `// @deno-types="npm:zod@3.23.8"` directive already at `supabase/functions/search/index.ts:15` shows the `npm:` specifier path is operative, so the original NI's "Deno-compatible import paths are non-trivial" reasoning has been partially eroded by Supabase Edge Function tooling maturing since the NI was written. NI-020 (`packages/shared/src/embedding/` barrel speculative exports) is held under this same trigger. Slice 7 introduced a third Edge Function (`invite-user`) that does not (yet) import from `packages/shared`; the case for unified Deno-side wiring strengthened with that addition.
- **Trigger:** Explicit pickup after Slice 7 ships clean and Slice 8 (demo mode) decisions land. Single-session CC slice; no SPEC changes anticipated beyond removal of this entry on resolution. Sequenced before B7 so the pre-launch test sweep exercises the unified Edge Function surface.

## B10 — Demo mode for portfolio access

- **What:** Unauthenticated `/demo` route, separate `demo` publication seeded with public-record meetings, demo signup affordance that uses the Slice 7 invitation infrastructure server-side (form takes email → server creates invitation for demo publication + calls `inviteUserByEmail` → trigger resolves user as a demo-publication reader on magic-link click). Banner on demo-scoped pages indicating demo framing. Real Midcoast Villager publication stays invitation-only.
- **Why:** Job-application demo surface. Prospective employers / recruiters can try the product end-to-end without coordinating with the developer. Pattern 1 from the Slice 7 demo-mode discussion: preserves the "private app" product lock by keeping the real publication invitation-only and routing demo access to a parallel tenant. The Slice 7 invitation schema, trigger, and resolution path are publication-agnostic specifically so this slice can layer on additively rather than reshaping Slice 7's data model.
- **Status note (2026-08-08):** largely superseded by ADR 0024's `public_read` flag, which delivers B10's stated purpose — a prospect exploring without coordinating with the developer — against the real publication instead of a seeded parallel tenant. B10 survives only if a reason emerges to demo against insulated content.
- **Trigger:** Pickup after Slice 7 ships and its audit/fixes close. Sub-decisions to resolve at planning time: content set (same Lincolnville Select Board meetings, a different Maine board for full insulation, or a hand-curated subset), demo signup gate (fully open with Cloudflare rate-limiting vs token-gated per recruiter), demo membership TTL (permanent vs expiring), and Aaron/Will clearance (required for same-content or hand-curated options). Sequenced before B7 so the pre-launch test sweep exercises the demo path.

## B12 — Investigate local `supabase db push --linked` desync with CI Migrate workflow

- **What:** Document and close the operational gap where running `supabase db push --linked` from a developer machine against cloud Supabase causes the CI Migrate workflow on subsequent merges to `main` to fail. Investigation surface: confirm whether local pushes write to `supabase_migrations.schema_migrations` in a way the CI workflow cannot reconcile, whether the failure pattern is "duplicate migration applied" or "missing baseline" or something else, and whether the resolution is a workflow change (CI should ignore migrations already applied), a developer-machine restriction (forbid `db push --linked`), or both. Update CLAUDE.md §6 and/or §5 with the resolved rule once the failure mode is characterized.
- **Why:** Surfaced during Slice 7 deploy. Recurring CI Migrate failures traced to local `db push --linked` invocations contaminating cloud schema-migration state. The current CLAUDE.md §6 migration rule mandates "Migrations: Supabase CLI. Migrations checked into `supabase/migrations/` and run from a GitHub Action on merge to `main`, never from worker boot." — this implicitly forbids local cloud pushes but doesn't say so explicitly, and the failure mode bit the Slice 7 cycle. Until investigation completes, a precautionary §6 rule lands in this update forbidding local `db push --linked`.
- **Trigger:** Next CI Migrate failure (paired with a developer-machine `db push --linked` invocation) provides the diagnostic data. Until then, the precautionary rule in CLAUDE.md §6 (added this cycle) bounds the surface; B12 closes when the actual failure mode is characterized and a permanent rule supersedes the precautionary one.
