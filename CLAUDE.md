...
- **Never run `supabase db push --linked` (or any command that applies migrations to cloud Supabase) from a developer machine.** Migrations apply to cloud through the GitHub Actions Migrate workflow on merge to `main`, exclusively. Local cloud pushes desync the cloud's `supabase_migrations.schema_migrations` table from the migration history CI tracks, surfacing as recurring CI Migrate failures on subsequent merges. This is a precautionary rule pending B12 investigation; once the exact failure mode is characterized the rule may tighten or be replaced. Local development against a local Supabase instance (`supabase db reset`, `supabase start`) is unaffected — only the `--linked` cloud variant is forbidden.
- **New Supabase Edge Functions require an explicit deploy step in `.github/workflows/deploy-functions.yml` in the same PR that introduces the function.** Adding a function under `supabase/functions/<name>/` without a matching `supabase functions deploy <name>` invocation in the workflow leaves the function un-deployed in cloud, silently. Slice 7's `invite-user` shipped this way and Aaron's admin invite flow was non-functional in production until the gap was caught manually. The `functions-deploy-coverage` job in `.github/workflows/ci.yml` enforces this at PR time: it fails the build if any `supabase/functions/<name>/` directory (leading-underscore shared dirs excepted) has no matching `supabase functions deploy <name>` line in the deploy workflow. Do not work around the job by editing it — add the deploy step.
